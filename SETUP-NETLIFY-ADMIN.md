...
Tutto si gestisce dal pannello `/admin/` — nessun upload manuale di file JSON.
...
- Branch: `main`
- Build command: `node scripts/verify-deploy-assets.js && node scripts/sync-site-data.js`
- Publish directory: `.` (root del repo, dove c’è `index.html`)
- **Importante:** la cartella `images/` con tutte le foto deve essere su GitHub (`git add images/`). Senza immagini nel repo il sito online non mostra le foto.
...
In `admin/config.yml` la riga `local_backend` è **commentata** per la produzione (usa Git Gateway su Netlify). Per sviluppo locale, decommentala e avvia `npx decap-server`.
